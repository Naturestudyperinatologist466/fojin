...
聚合 409 个活跃数据源（覆盖 29 个国家/地区）、8,949 条目录记录、237,593 条辞典词条、28 语种。可通过典津联检扩展至 72.8 万条跨平台古籍资源。

> **佛津 vs 典津**：典津聚合的是"汉籍影像"元数据（只能跳转到外部平台查看），而佛津深耕佛教领域，提供**经文全文阅读、多语种对读、辞典检索、知识图谱**等一站式研究功能——不只是"找到"，更能"读到、查到、关联到"。
...
| **多语种对读** | ✅ 汉/梵/巴利/藏/英 28 语种平行对读 | ❌ 仅汉籍 |
| **辞典检索** | ✅ 6 部权威辞典、237,593 条词条（中/梵/巴/英） | ❌ 无辞典功能 |
| **知识图谱** | ✅ 9,600+ 实体、3,800+ 关系，力导向图可视化 | ❌ 无知识图谱 |
| **写本浏览** | ✅ IIIF 协议对接 BDRC 等写本影像 | ❌ 无 IIIF 写本浏览 |
| **数据源数量** | 409 个佛教专题数据源，29 国/地区 | ~180 个通用汉籍数据源 |
| **语种覆盖** | 28 语种（含梵文、巴利文、藏文、犍陀罗语等） | 以汉文为主 |
| **联合检索** | ✅ 本地 + 典津 72.8 万条联检 | 仅自身数据 |
...
│   ├── alembic/versions/  # 数据库迁移（0001–0075）
│   ├── scripts/           # 数据导入脚本
...
迁移文件位于 `backend/alembic/versions/`，当前最新为 `0075`。
